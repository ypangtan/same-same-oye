...
class RadioQueueService {
     ...
     * timestamps are kept afterwards, as the played-history record. Since this fires at the
     * start of the track and there's nothing after it until the next one starts, "most recently
     * played" doubles as "currently on air" — see nowPlaying().
     */
    ...
    /**
     * Live "now playing" panel for the backoffice: the most recently aired track (see the note
     * on markPlayed() for why that doubles as "currently on air"), plus the current live
     * listener count read straight from Icecast.
     */
    public static function nowPlaying() {
        ...
        return response()->json( [
            ...
            'played_at' => $current ? $current->played_at->timezone( 'Asia/Kuala_Lumpur' )->format( 'Y-m-d H:i:s' ) : null,
