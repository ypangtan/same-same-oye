<div class="nk-block-head nk-block-head-sm">
    <div class="nk-block-between">
        <div class="nk-block-head-content">
            <h3 class="nk-block-title page-title">{{ __( 'template.subscription_plans' ) }}</h3>
        </div><!-- .nk-block-head-content -->
        @can( 'add subscription_plans' )
        <div class="nk-block-head-content">
                ...
                <div class="toggle-expand-content" data-content="pageMenu">
                    <ul class="nk-block-tools g-3">
                        <li class="nk-block-tools-opt">
                            <a href="{{ route( 'admin.subscription_plan.add' ) }}" class="btn btn-primary">{{ __( 'template.add' ) }}</a>
                        </li>
                    </ul>
                </div>
            </div>
...
$columns = [
    ...
    [
        'type' => 'input',
        'placeholder' =>  __( 'datatables.search_x', [ 'title' => __( 'subscription_plan.name' ) ] ),
        'id' => 'name',
        'title' => __( 'subscription_plan.name' ),
    ],
    [
        'type' => 'default',
        'id' => 'price',
        'title' => __( 'subscription_plan.price' ),
    ],
    [
        'type' => 'default',
        'id' => 'duration_in_days',
...
<x-data-tables id="subscription_plan_table" enableFilter="true" enableFooter="false" columns="{{ json_encode( $columns ) }}" />
...
var statusMapper = {
    ...
    dt_table,
    dt_table_name = '#subscription_plan_table',
    dt_table_config = {
        language: {
    ...
    document.addEventListener( 'DOMContentLoaded', function() {
        ...
        $( document ).on( 'click', '.dt-edit', function() {
            window.location.href = '{{ route( 'admin.subscription_plan.edit' ) }}?id=' + $( this ).data( 'id' );
        } );

        $( document ).on( 'click', '.dt-status', function() {

            $.ajax( {
                url: '{{ route( 'admin.subscription_plan.updateSubscriptionPlanStatus' ) }}',
                type: 'POST',
                data: {
                    'id': $( this ).data( 'id' ),
