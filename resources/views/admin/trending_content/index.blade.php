<div class="nk-block-head nk-block-head-sm">
    <div class="nk-block-between">
        <div class="nk-block-head-content">
            <h3 class="nk-block-title page-title">{{ __( 'template.trending_contents' ) }}</h3>
        </div><!-- .nk-block-head-content -->
        @can( 'add trending_contents' )
        <div class="nk-block-head-content">
                ...
                <div class="toggle-expand-content" data-content="pageMenu">
                    <ul class="nk-block-tools g-3">
                        <li class="nk-block-tools-opt">
                            <a href="{{ route( 'admin.module_parent.trending_content.add' ) . '?type=' . $type . '&parent_route=' . $parent_route }}" class="btn btn-primary">{{ __( 'template.add' ) }}</a>
                        </li>
                    </ul>
                </div>
            </div>
...
$columns = [
    ...
    [
        'type' => 'input',
        'placeholder' =>  __( 'datatables.search_x', [ 'title' => __( 'trending_content.title' ) ] ),
        'id' => 'title',
...
var statusMapper = @json( $data['status'] ),
    dt_table,
    dt_table_name = '#trending_content_table',
    dt_table_config = {
        language: {
    ...
    document.addEventListener( 'DOMContentLoaded', function() {
        ...
        $( document ).on( 'click', '.dt-edit', function() {
            window.location.href = '{{ route( 'admin.trending_content.edit' ) }}?id=' + $( this ).data( 'id' );
        } );

        $( document ).on( 'click', '.dt-status', function() {

            $.ajax( {
                url: '{{ route( 'admin.trending_content.updateTrendingContentStatus' ) }}',
                type: 'POST',
                data: {
                    'id': $( this ).data( 'id' ),
