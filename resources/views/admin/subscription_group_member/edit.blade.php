            ...
            $.ajax( {
                ...
                success: function( response ) {
                    ...
                    if( response.leader != null ){
                        name = ( response.leader.phone_number ? ( response.leader.calling_code ? response.leader.calling_code : '+60' ) + ( response.leader.phone_number ? response.leader.phone_number : '-' ) : '' )
                        + ' (' + ( response.leader.email ? response.leader.email : '-' ) + ')';
                        let option2 = new Option( name, response.leader.encrypted_id, true, true );
                        leaderSelect2.append( option2 );
