<?php 
    $website_banners = $data['website_banners'];
?>

<style>
    .sortable-placeholder {
        background: #f8f9fa;
        border: 2px dashed #ccc;
        height: 100px;
    }
    .website_banner-img {
        width: 80%; /* ✅ Increased size */
        height: 120px;
        object-fit: contain; /* ✅ Ensures it maintains aspect ratio */
    }
    .list-group-item {
        display: flex;
        justify-content: center; /* ✅ Centers content */
        align-items: center;
        text-align: center;
    }

    #website_banner-list {
        display: grid;
        grid-template-columns: repeat(3, 1fr);
        gap: 10px;
        padding: 0;
    }
    #website_banner-list .list-group-item {
        width: 100%;
        text-align: center;
        background: #f8f9fa;
        border-radius: 8px;
        padding: 10px;
    }
    .website_banner-img {
        width: 100%;
        max-width: 150px;
        object-fit: cover;
    }

    #website_banner-list li:hover {
        background: #e9ecef;
        box-shadow: 0px 4px 8px rgba(0, 0, 0, 0.1);
    }

    .sortable-placeholder {
        background: #dee2e6;
        border: 2px dashed #6c757d;
        height: 130px;
        border-radius: 8px;
        margin-bottom: 10px;
    }

</style>

<?php $website_banner_create = 'website_banner_create'; ?>

<div class="nk-block-head nk-block-head-sm">
    <div class="nk-block-between">
        <div class="nk-block-head-content">
            <h3 class="nk-block-title page-title">{{ __( 'template.add_x', [ 'title' => Str::singular( __( 'template.website_banners' ) ) ] ) }}</h3>
        </div>
    </div>
</div>

<div class="card">
    <div class="card-inner">
        <h5 class="card-title mb-4">{{ __( 'template.general_info' ) }}</h5>
        @can( 'add website_banners' )
        <div class="mb-3">
            <label>{{ __( 'banner.image' ) }}</label>
            <div class="dropzone mb-3" id="{{ $website_banner_create }}_image" style="min-height: 0px;">
                <div class="dz-message needsclick">
                    <h3 class="fs-5 fw-bold text-gray-900 mb-1">{{ __( 'template.drop_file_or_click_to_upload' ) }}</h3>
                </div>
            </div>
            <div class="invalid-feedback"></div>
        </div>
        @endcan
        <ul id="website_banner-list" class="list-group">
            @foreach($website_banners as $website_banner)
                <li class="list-group-item d-flex flex-column align-items-center justify-content-center position-relative" data-id="{{ $website_banner->id }}">
                    <img src="{{ $website_banner->image_path }}" class="website_banner-img rounded">
                    <!-- Dropdown -->
                    @can( 'edit website_banners' )
                    <div class="dropdown mt-2">
                        <button class="btn btn-secondary btn-sm dropdown-toggle" type="button" data-bs-toggle="dropdown" aria-expanded="false">
                            <em class="icon ni ni-more-h"></em>
                        </button>
                        <ul class="dropdown-menu">
                            <li>
                                <button class="dropdown-item edit-website_banner" data-id="{{ $website_banner->id }}">{{ __( 'template.edit' ) }}</button>
                            </li>
                            <li>
                                <button class="dropdown-item text-danger delete-website_banner" data-id="{{ $website_banner->id }}">Delete</button>
                            </li>
                        </ul>
                    </div>
                    @endcan
                </li>
            @endforeach
        </ul>
        
    </div>
</div>

<!-- jQuery (Make sure jQuery is included before jQuery UI) -->
<script src="https://code.jquery.com/jquery-3.6.0.min.js"></script>

<script src="https://ajax.googleapis.com/ajax/libs/jqueryui/1.10.3/jquery-ui.min.js"></script>

<!-- jQuery UI CSS (Optional, for better styling) -->
<link rel="stylesheet" href="https://code.jquery.com/ui/1.13.2/themes/smoothness/jquery-ui.css">
<script src="https://cdn.jsdelivr.net/npm/sortablejs@latest/Sortable.min.js"></script>

<script>
    document.addEventListener('DOMContentLoaded', function() {

    let fc = '#{{ $website_banner_create }}', fileID = '';
    let website_bannerUrlTimer = {};
    let website_bannerPublishingDateTimer = {};

    $(fc + '_cancel').click(() => window.location.href = '{{ route('admin.module_parent.website_banner.index') }}');

    // ✅ Prevent Dropzone from being attached multiple times
    if (Dropzone.instances.length > 0) {
        Dropzone.instances.forEach(dz => dz.destroy()); // Destroy existing Dropzones before initializing
    }

    // ✅ Ensure Dropzone is initialized once
    if (!$(fc + '_image').hasClass("dz-clickable")) {
        Dropzone.autoDiscover = false;
        let myDropzone = new Dropzone(fc + '_image', {
            url: "{{ route('admin.website_banner.createWebsiteBanner') }}",
            maxFiles: 1,
            acceptedFiles: 'image/*,.heic,.heif,.webp',
            addRemoveLinks: true,
            params: {
                _token: "{{ csrf_token() }}"
            },
            success: function(file, response) {
                if (response.status == 200) {
                    let newWebsiteBanner = $(`
                        <li class="list-group-item d-flex flex-column align-items-center justify-content-center position-relative" data-id="${response.data.id}${response.data.id}">
                            <img src="${response.data.website_banner_url}" class="website_banner-img rounded">
                            <!-- Dropdown -->
                            <div class="dropdown mt-2">
                                <button class="btn btn-secondary btn-sm dropdown-toggle" type="button" data-bs-toggle="dropdown" aria-expanded="false">
                                    <em class="icon ni ni-more-h"></em>
                                </button>
                                <ul class="dropdown-menu">
                                    <li>
                                        <button class="dropdown-item text-danger delete-website_banner" data-id="${response.data.id}">Delete</button>
                                    </li>
                                </ul>
                            </div>
                        </li>
                    `);
                    $("#website_banner-list").append(newWebsiteBanner);

                    // Init flatpickr on newly added publishing_date input
                    flatpickr(newWebsiteBanner.find('.website_banner_publishing_date')[0], {

                        dateFormat: 'Y-m-d',
                        disableMobile: true,
                        allowInput: true,
                    });
                    // Remove file preview after upload so dropzone is ready for the next image
                    setTimeout(function() {
                        myDropzone.removeFile(file);
                    }, 300);
                }

            }
        });
    }

    // ✅ Initialize Sortable.js
    let sortableList = new Sortable(document.getElementById('website_banner-list'), {
        animation: 200, // Smooth transition effect
        handle: ".website_banner-img", // Users can drag by clicking on the image
        ghostClass: 'sortable-placeholder', // Placeholder class when dragging
        handle: ".list-group-item", // Only drag using the list items
        ghostClass: "sortable-placeholder", // CSS class for dragged element
        onEnd: function(evt) {
            let sortedIDs = [];
            $("#website_banner-list li").each(function() {
                if( $(this).data("id") ){
                    sortedIDs.push($(this).data("id"));
                }
            });

            // ✅ Send updated order to backend
            $.ajax({
                url: "{{ route('admin.website_banner.updateOrder') }}",
                type: "POST",
                data: {
                    _token: "{{ csrf_token() }}",
                    order: sortedIDs
                },
                success: function(response) {
                    console.log("WebsiteBanner order updated successfully!");
                },
                error: function(error) {
                    console.error("Error updating website_banner order", error);
                }
            });
        }
    });

    $( document ).on( 'click', '.edit-website_banner', function() {
        window.location.href = '{{ route( 'admin.website_banner.edit' ) }}?id=' + $( this ).data( 'id' );
    } );

    // ✅ Delete WebsiteBanner
    $(document).on("click", ".delete-website_banner", function() {
        let website_bannerId = $(this).data("id");
        let website_bannerItem = $(this).closest(".list-group-item"); // Ensure correct targeting

        $( 'body' ).loading( {
            message: '{{ __( 'template.loading' ) }}'
        } );

        $.post('{{ route("admin.website_banner.updateWebsiteBannerStatus") }}', {
            _token: '{{ csrf_token() }}',
            id: website_bannerId
        }).done(function(response) {
            $( 'body' ).loading( 'stop' );

            website_bannerItem.fadeOut(300, function() {
                $(this).remove();
            });
        }).fail(function() {
            $( 'body' ).loading( 'stop' );

            alert("Error occurred. Please check your connection.");
        });
    });


});

</script>
