<div class="nk-block-head nk-block-head-sm">
    <div class="nk-block-between">
        <div class="nk-block-head-content">
            <h3 class="nk-block-title page-title">{{ __( 'template.vending_machines' ) }}</h3>
        </div><!-- .nk-block-head-content -->
        @can( 'add vending_machines' )
        <div class="nk-block-head-content">
                ...
                <div class="toggle-expand-content" data-content="pageMenu">
                    <ul class="nk-block-tools g-3">
                        <li class="nk-block-tools-opt">
                            <a href="{{ route( 'admin.vending_machine.add' ) }}" class="btn btn-primary">{{ __( 'template.add' ) }}</a>
                        </li>
                    </ul>
                </div>
            </div>
...
$columns = [
    ...
    [
        'type' => 'default',
        'id' => 'image',
        'title' => __( 'vending_machine.image' ),
    ],
    [
        'type' => 'input',
        'placeholder' =>  __( 'datatables.search_x', [ 'title' => __( 'vending_machine.title' ) ] ),
        'id' => 'title',
        'title' => __( 'vending_machine.title' ),
    ],
    [
        'type' => 'input',
        'placeholder' =>  __( 'datatables.search_x', [ 'title' => __( 'vending_machine.code' ) ] ),
        'id' => 'code',
        'title' => __( 'vending_machine.code' ),
    ],
    [
        'type' => 'default',
        'id' => 'api_key',
        'title' => __( 'vending_machine.api_key' ),
    ],
    [
        'type' => 'select',
        'options' => $data['status'],
...
<x-data-tables id="vending_machine_table" enableFilter="true" enableFooter="false" columns="{{ json_encode( $columns ) }}" />
...
var statusMapper = @json( $data['status'] ),
    dt_table,
    dt_table_name = '#vending_machine_table',
    dt_table_config = {
        language: {
        ...
        columnDefs: [
            ...
            {
                targets: parseInt( '{{ Helper::columnIndex( $columns, "parent_vending_machine" ) }}' ),
                width: '10%',
                render: function( data, type, row, meta ) {
                    return data ? data : '-' ;
                },
            ...
            },
            {
                ...
                render: function( data, type, row, meta ) {
                    ...
                    @canany( [ 'edit vending_machines', 'delete vending_machines' ] )
                    let edit, status = '';

                    @can( 'edit vending_machines' )
                    edit = '<li class="dt-edit" data-id="' + row['encrypted_id'] + '"><a href="#"><em class="icon ni ni-edit"></em><span>{{ __( 'template.edit' ) }}</span></a></li>';
                    @endcan

                    @can( 'delete vending_machines' )
    ...
    document.addEventListener( 'DOMContentLoaded', function() {
        ...
        $( document ).on( 'click', '.dt-edit', function() {
            window.location.href = '{{ route( 'admin.vending_machine.edit' ) }}?id=' + $( this ).data( 'id' );
        } );

        $( document ).on( 'click', '.dt-status', function() {

            $.ajax( {
                url: '{{ route( 'admin.vending_machine.updateVendingMachineStatus' ) }}',
                type: 'POST',
                data: {
                    'id': $( this ).data( 'id' ),
