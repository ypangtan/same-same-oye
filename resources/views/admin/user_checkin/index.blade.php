                ...
                <div class="toggle-expand-content" data-content="pageMenu">
                    <ul class="nk-block-tools g-3">
                        <li class="nk-block-tools-opt">
                            <a href="{{ route( 'admin.user_checkin.add' ) }}" class="btn btn-primary">{{ __( 'template.add' ) }}</a>
                        </li>
                    </ul>
                </div>
            </div>
...
$columns = [
    ...
    [
        'type' => 'date',
        'placeholder' => __( 'datatables.search_x', [ 'title' => __( 'user_checkin.checkin_date' ) ] ),
        'id' => 'checkin_date',
        'title' => __( 'user_checkin.checkin_date' ),
    ],
    [
        'type' => 'input',
        'placeholder' =>  __( 'datatables.search_x', [ 'title' => __( 'user_checkin.user' ) ] ),
        'id' => 'user',
        'title' => __( 'user_checkin.user' ),
    ],
    [
        'type' => 'select',
        'options' => $data['status'],
...
<x-data-tables id="user_checkin_table" enableFilter="true" enableFooter="false" columns="{{ json_encode( $columns ) }}" />
...
var statusMapper = @json( $data['status'] ),
    dt_table,
    dt_table_name = '#user_checkin_table',
    dt_table_config = {
        language: {
        ...
        ajax: {
            url: '{{ route( 'admin.user_checkin.allUserCheckins' ) }}',
            data: {
                '_token': '{{ csrf_token() }}',
            },
            dataSrc: 'user_checkins',
        },
        ...
        columnDefs: [
            ...
            {
                targets: parseInt( '{{ Helper::columnIndex( $columns, "parent_user_checkin" ) }}' ),
                width: '10%',
                render: function( data, type, row, meta ) {
                    return data ? data : '-' ;
                },
            ...
            },
            {
                ...
                render: function( data, type, row, meta ) {
                    ...
                    @canany( [ 'edit user_checkins', 'delete user_checkins' ] )
                    let edit, status = '';

                    @can( 'edit user_checkins' )
    ...
    document.addEventListener( 'DOMContentLoaded', function() {
        ...
        $( document ).on( 'click', '.dt-edit', function() {
            window.location.href = '{{ route( 'admin.user_checkin.edit' ) }}?id=' + $( this ).data( 'id' );
        } );

        $( document ).on( 'click', '.dt-status', function() {

            $.ajax( {
                url: '{{ route( 'admin.user_checkin.updateUserCheckinStatus' ) }}',
                type: 'POST',
                data: {
                    'id': $( this ).data( 'id' ),
