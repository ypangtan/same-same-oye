<div class="nk-block-head nk-block-head-sm">
    <div class="nk-block-between">
        <div class="nk-block-head-content">
            <h3 class="nk-block-title page-title">{{ __( 'template.pop_announcements' ) }}</h3>
        </div><!-- .nk-block-head-content -->
        @can( 'add pop_announcements' )
        <div class="nk-block-head-content">
                ...
                <div class="toggle-expand-content" data-content="pageMenu">
                    <ul class="nk-block-tools g-3">
                        <li class="nk-block-tools-opt">
                            <a href="{{ route( 'admin.pop_announcement.add' ) }}" class="btn btn-primary">{{ __( 'template.add' ) }}</a>
                        </li>
                    </ul>
                </div>
            </div>
...
$columns = [
    ...
    [
        'type' => 'default',
        'id' => 'image',
        'title' => __( 'pop_announcement.image' ),
    ],
    [
        'type' => 'input',
        'placeholder' =>  __( 'datatables.search_x', [ 'title' => __( 'pop_announcement.title' ) ] ),
        'id' => 'title',
        'title' => __( 'pop_announcement.title' ),
    ],
    [
        'type' => 'select',
        'options' => $data['status'],
...
<x-data-tables id="pop_announcement_table" enableFilter="true" enableFooter="false" columns="{{ json_encode( $columns ) }}" />
...
var statusMapper = {
    ...
    dt_table,
    dt_table_name = '#pop_announcement_table',
    dt_table_config = {
        language: {
        ...
        columnDefs: [
            ...
            },
            {
                ...
                render: function( data, type, row, meta ) {
                    ...
                    @canany( [ 'edit pop_announcements', 'delete pop_announcements' ] )
                    let edit, status = '';

                    @can( 'edit pop_announcements' )
                    edit = '<li class="dt-edit" data-id="' + row['encrypted_id'] + '"><a href="#"><em class="icon ni ni-edit"></em><span>{{ __( 'template.edit' ) }}</span></a></li>';
                    @endcan

                    @can( 'delete pop_announcements' )
    ...
    document.addEventListener( 'DOMContentLoaded', function() {
        ...
        $( document ).on( 'click', '.dt-edit', function() {
            window.location.href = '{{ route( 'admin.pop_announcement.edit' ) }}?id=' + $( this ).data( 'id' );
        } );
