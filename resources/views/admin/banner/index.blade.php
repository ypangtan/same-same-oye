<div class="nk-block-head nk-block-head-sm">
    <div class="nk-block-between">
        <div class="nk-block-head-content">
            <h3 class="nk-block-title page-title">{{ __( 'template.vouchers' ) }}</h3>
                ...
                <div class="toggle-expand-content" data-content="pageMenu">
                    <ul class="nk-block-tools g-3">
                        <li class="nk-block-tools-opt">
                            <a href="{{ route( 'admin.voucher.add' ) }}" class="btn btn-primary">{{ __( 'template.add' ) }}</a>
                        </li>
                    </ul>
                </div>
            </div>
...
<div class="container">
    <h3 class="text-xl font-bold mb-4">{{ __( 'Banners' ) }}</h3>
    <ul id="sortable" class="list-none p-0">
        @foreach( $banners as $banner )
            <li class="p-3 border rounded mb-2 bg-white cursor-move" data-id="{{ $banner->id }}">
                <img src="{{ asset( 'storage/' . $banner->image ) }}" class="w-20 h-20 object-cover rounded">
            </li>
        @endforeach
    </ul>
</div>
...
var statusMapper = @json( $data['status'] ),
        ...
        columnDefs: [
            ...
            },
            {
                ...
                render: function( data, type, row, meta ) {
                    ...
                    @canany( [ 'edit vouchers', 'delete vouchers' ] )
                    let edit, status = '';
    ...
    document.addEventListener( 'DOMContentLoaded', function() {
        ...
        $( document ).on( 'click', '.dt-edit', function() {
            window.location.href = '{{ route( 'admin.voucher.edit' ) }}?id=' + $( this ).data( 'id' );
        } );

        $( document ).on( 'click', '.dt-status', function() {

            $.ajax( {
                url: '{{ route( 'admin.voucher.updateVoucherStatus' ) }}',
                type: 'POST',
                data: {
                    'id': $( this ).data( 'id' ),
